...
static void draw_hourly_y_ticks(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx, lv_hourly_chart_axis_t axis)
{
    ...
    for(uint32_t i = 0; i < ticks_cnt; i++) {
        ...
        lv_snprintf(buf, sizeof(buf), "%ld", tick_value);
...
static void draw_hourly_x_ticks(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    for(uint32_t i = 0; i < NUM_HOURS; i++) { 
        ...
        // 🔥 показваме през 2 часа
        if (hour % 2 != 0) continue;

        char buf[8];

        // ако искаш 24 вместо 00
        int display_hour = (hour == 0) ? 24 : hour;

        lv_snprintf(buf, sizeof(buf), "%02d", display_hour);
        ...
        lv_point_t size;
        lv_txt_get_size(&size, buf, label_dsc.font,
...
static void draw_hourly_div_lines(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    for(uint32_t i = 0; i < NUM_HOURS; i++) { 
        ...
        else if (dt.tm_hour == 4 || dt.tm_hour == 8 ||
				dt.tm_hour == 16 || dt.tm_hour == 20) {
