...
bool weather_http_fetch_main(float lat, float lon, weather_data_t *out)
{
    ...
    int len = http_get(url, buf, sizeof(buf));
    if (len <= 0) {
        ESP_LOGE(TAG, "HTTP GET failed (main)");
        return false;
    }

    ESP_LOGI(TAG, "Weather HTTP len=%d", len);

    cJSON *root = cJSON_ParseWithLength(buf, len);
    if (!root) {
        ESP_LOGE(TAG, "JSON parse failed (main)");
        return false;
    }

    // ... тук остава твоята логика за current/hourly/daily ...
    // =========================
    // CURRENT
    // =========================
...
snprintf(url, sizeof(url),
    "http://marine-api.open-meteo.com/v1/marine?"
    "latitude=%.6f&longitude=%.6f"
    ...
    int len = http_get(url, buf, sizeof(buf));
    if (len <= 0) {
        ESP_LOGE(TAG, "HTTP GET failed (sea)");
        out->has_sea = 0;
        return true;
    }

    ESP_LOGI(TAG, "Sea HTTP len=%d", len);

    cJSON *root = cJSON_ParseWithLength(buf, len);
    if (!root) {
        ESP_LOGE(TAG, "JSON parse failed (sea)");
        out->has_sea = 0;
        return true;
    }
    ...
    if (v && cJSON_IsNumber(v)) {
        out->sea_temp = (float)v->valuedouble;
        out->has_sea = 1;

        ESP_LOGI(TAG, "Sea temperature: %.1f °C", out->sea_temp);
    } else {
        out->has_sea = 0;
        ESP_LOGI(TAG, "Sea temperature unavailable");
    }
